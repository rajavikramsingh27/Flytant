...
class SINCallKitProvider: NSObject, CXProviderDelegate {
    ...
    init(withClient: SINClient) {
        ...
        let config = CXProviderConfiguration(localizedName: "Flytant")
...
//        NotificationCenter.default.addObserver(self, selector: #selector(callDidEnd(_:)), name: NSNotification.Name(rawValue: "SINCallDidEndNotification"), object: nil)
        
    }
    
    func reportNewIncomingCall (call: SINCall) {
        var caller = "WChat Call"
        if let call = call.headers["username"] {
...
//    @objc func callDidEnd(_ sender: Notification) {
//
//        if let call: SINCall = sender.userInfo![SINCallKey] as? SINCall {
//
