...
private let reuseIdentifier = "likesCollectionViewCell"

class LikesVC: UICollectionViewController, UICollectionViewDelegateFlowLayout, LikeCellDelegate {
...
//        navigationController?.navigationBar.setGradientBackground(colors: [UIColor(red:250/255, green:0/255, blue:102/255, alpha:1), UIColor(red:212/255, green:24/255, blue: 114/255, alpha:0.1), UIColor(red:148/255, green:0/255, blue: 211/255, alpha:0.1)], startPoint: .topLeft, endPoint: .topRight)
    ...
    private func configureCollectionView(){
        self.collectionView.register(LikesCollectionViewCell.self, forCellWithReuseIdentifier: reuseIdentifier)
...
//    MARK: - CollectionView Dlelegate and Data Source
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as! LikesCollectionViewCell
        cell.delegate = self
...
//    MARK: - API
    ...
    private func fetchLikesUsers(){
        guard let usersLiked = self.usersLiked else {return}
        if self.index < usersLiked.count{
            fetchUsersData(user: usersLiked[index]) { (success, error) in
                if success{
                    self.index += 1
                    self.fetchLikesUsers()
                }else{
                    self.fetchLikesUsers()
                }
            }
    ...
    func configureFollow(indexPath: IndexPath, cell: LikesCollectionViewCell){
        if !users.isEmpty{
